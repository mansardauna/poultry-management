...
async function testSystemSettings() {
  ...
  const { data: upsertData, error: upsertErr } = await supabase.from('systemSettings').upsert([{
    id: 'saas_plans_config',
    workspaceId: 'global',
    adminName: JSON.stringify(DEFAULT_PLANS)
  }]);

  console.log("Upsert error:", upsertErr);

  const { data: readData, error: readErr } = await supabase.from('systemSettings').select('*').eq('id', 'saas_plans_config').single();
  console.log("Read error:", readErr);
  console.log("Read data stored plans:", readData ? JSON.parse(readData.adminName) : null);
}
